(function($) {
	...
	elFinder = function(el, o) {
		...
		this.select = function(el, reset) {
			...
			if( el.hasClass('ui-selected') ){ 				// goodlayers mod
				el.gal_img_clicked( this.cwd.rel );			// goodlayers mod
			} 												// goodlayers mod
...
elFinder.prototype.view = function(fm, el) {
	...
	this.cwd_wrap = $('<div class="cwd-wrapper" />'); 									// goodlayers mod
		this.galdb = $('<div class="el-finder-galdb" id="el-finder-galdb" />'); 		// goodlayers mod
		this.galdb.init_galdb();														// goodlayers mod
		this.cwd = $('<div class="el-finder-cwd" />').attr('unselectable', 'on'); 
		this.cwd_wrap.append(this.galdb).append(this.cwd); 								// goodlayers mod
	...
	this.wrz = $('<div class="el-finder-workzone" />')
		.append(this.nav)
		.append(this.cwd_wrap) 															// goodlayers mod
		.append(this.spn)
		.append(this.err)
	...
	*/
	this.spinner = function(show) {
		...
		this.galdb.toggleClass('el-finder-disabled', show);
	...
	*/
	this.renderNav = function(tree) {
		...
		var d = tree.dirs.length ? traverse(tree.dirs, 'Gallery') : '', // Goodlayers mod
		...
		function traverse(tree, parents) { // goodlayers mod
			...
			for (i=0; i < tree.length; i++) {
				...
				var gal_label_attr = ' data-type="directory" data-parents="' + parents + '" data-name="' + tree[i].name + '" ';  // goodlayers mod
				html += '<li><a href="#" class="'+c+' gal-label-attr" ' + gal_label_attr + ' key="'+tree[i].hash+'"><div'+(tree[i].dirs.length ? ' class="collapsed"' : '')+'/>'+tree[i].name+'</a>'; // goodlayers mod

				if (tree[i].dirs.length) {
					html += traverse(tree[i].dirs, parents + '\\' + tree[i].name  ); // goodlayers mod
				}
				html += '</li>';
			}
			return html +'</ul>';
	...
	*/
	this.renderIcon = function(f) {
		var gal_label_attr = ' data-type="' + f.mime + '" data-name="' + f.name + '" '; 																				//goodlayers mod
		var str = '<p'+(f.tmb ? ' style="'+"background:url('"+f.tmb+"') 0 0 no-repeat"+'"' : '')+'/><label class="gal-label-attr" ' + gal_label_attr + '>'+this.formatName(f.name)+'</label>'; //goodlayers mod
	...
	*/
	this.renderRow = function(f, odd) {
		...
		var gal_label_attr = ' data-type="' + f.mime + '" data-name="' + f.name + '" ';  // Goodlayers mod
		return '<tr key="'+f.hash+'" class="'+self.mime2class(f.mime)+(odd ? ' el-finder-row-odd' : '')+'"><td class="icon gal-label-attr" ' + gal_label_attr + '><p>'+str+'</p></td><td>'+f.name+'</td><td>'+self.formatPermissions(f.read, f.write, f.rm)+'</td><td>'+self.formatDate(f.date)+'</td><td class="size">'+self.formatSize(f.size)+'</td><td>'+self.mime2kind(f.link ? 'symlink' : f.mime)+'</td></tr>'; // Goodlayers mod
	}
...
elFinder.prototype.ui.prototype.commands = {
	...
	open : function(fm) {
		...
		this.exec = function(dir) {
			...
			function openDir(id) {
				var data_parents = $(dir).attr('data-parents'); 	// goodlayers mod
				$(dir).parent().gal_img_clicked( data_parents );	 // goodlayers mod
